...
def process_projectslug(project_slug):
    try:
        ...
        # New Users
        observers_df['user_creation'] = pd.to_datetime(
            observers_df['user'].apply(lambda x: x['created_at'])
        ...
        print(f"Exception occurred while processing project {project_slug}: {e}")
        return {"error": "Failed to fetch data from iNaturalist API."}
